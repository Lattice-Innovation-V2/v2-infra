"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getTableIamPolicyOutput = exports.getTableIamPolicy = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * Retrieves the current IAM policy data for a Bigtable Table.
 *
 * ## example
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as gcp from "@pulumi/gcp";
 *
 * const policy = gcp.bigtable.getTableIamPolicy({
 *     instanceName: instance.name,
 *     table: table.name,
 * });
 * ```
 */
function getTableIamPolicy(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("gcp:bigtable/getTableIamPolicy:getTableIamPolicy", {
        "instanceName": args.instanceName,
        "project": args.project,
        "table": args.table,
    }, opts);
}
exports.getTableIamPolicy = getTableIamPolicy;
/**
 * Retrieves the current IAM policy data for a Bigtable Table.
 *
 * ## example
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as gcp from "@pulumi/gcp";
 *
 * const policy = gcp.bigtable.getTableIamPolicy({
 *     instanceName: instance.name,
 *     table: table.name,
 * });
 * ```
 */
function getTableIamPolicyOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("gcp:bigtable/getTableIamPolicy:getTableIamPolicy", {
        "instanceName": args.instanceName,
        "project": args.project,
        "table": args.table,
    }, opts);
}
exports.getTableIamPolicyOutput = getTableIamPolicyOutput;
//# sourceMappingURL=getTableIamPolicy.js.map