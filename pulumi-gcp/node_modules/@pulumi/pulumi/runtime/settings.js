 ...
 */
function createTraceContextInterceptor() {
...
async function awaitFeatureSupport() {
    ...
    if (monitorRef !== undefined) {
        const store = state_1.getStore();
        const [secrets, resourceReferences, outputValues, deletedWith, replaceWith, aliasSpecs, transforms, invokeTransforms, parameterization, resourceHooks, errorHooks,] = await Promise.all([
            "secrets",
            "resourceReferences",
            "outputValues",
            "deletedWith",
...
function requirePulumiVersion(range) {
    ...
    return new Promise((resolve, reject) => {
        engineRef.requirePulumiVersion(req, (err, resp) => {
            if (err && err.code === grpc.status.UNIMPLEMENTED) {
                return reject(new Error("The installed version of the CLI does not support the `RequirePulumiVersion` RPC. " +
    ...
    if (state_1.getLocalStore() === undefined && addr !== "mock") {
        if (monitor === undefined) {
            if (addr) {
                ...
                monitor = new resrpc.ResourceMonitorClient(addr, grpc.credentials.createInsecure(), exports.grpcClientOptions);
        ...
        if (settings.monitor === undefined) {
            if (addr) {
                // Lazily initialize the RPC connection to the monitor.
                settings.monitor = new resrpc.ResourceMonitorClient(addr, grpc.credentials.createInsecure(), exports.grpcClientOptions);
                settings.options.monitorAddr = addr;
            }
        }
    ...
    if (state_1.getLocalStore() === undefined) {
        if (engine === undefined) {
            ...
            if (addr) {
                ...
                engine = new engrpc.EngineClient(addr, grpc.credentials.createInsecure(), exports.grpcClientOptions);
            }
        }
        return engine;
    }
        ...
        if (settings.engine === undefined) {
            ...
            if (addr) {
                // Lazily initialize the RPC connection to the engine.
                settings.engine = new engrpc.EngineClient(addr, grpc.credentials.createInsecure(), exports.grpcClientOptions);
            }
        }
