...
const provider_1 = require("../../provider/experimental/provider");
...
function run(opts) {
    ...
    const programArgs = opts.argv._.slice(1);
...
${errMsg}`);
    ...
    const runProgram = async () => {
        ...
        try {
            ...
            const components = provider_1.getPulumiComponents(reqResult);
            if (components.length > 0) {
                ...
                if (!/^[a-zA-Z]/.test(providerName)) {
                    throw new Error(`Invalid provider name '${providerName}' in package.json. Provider names must start with a letter.`);
                }
                ...
                return await provider_1.componentProviderHost({
                    name: providerName,
                    namespace: namespace,
                    dirname: program,
                    components,
