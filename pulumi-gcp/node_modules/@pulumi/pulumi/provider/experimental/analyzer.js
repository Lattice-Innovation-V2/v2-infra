...
Please ensure these components are properly imported to your package's entry point.`);
    ...
    analyzeComponent(node) {
        ...
        if (argsSymbol.members) {
            inputs = this.analyzeSymbols({ component: componentName, inputOutput: InputOutput.Neither, typeName: argsSymbol.getName() }, symbolTableToSymbols(argsSymbol.members));
        }
        let outputs = {};
        const classType = this.checker.getTypeAtLocation(node);
        const classSymbol = classType.getSymbol();
        if (classSymbol?.members) {
            outputs = this.analyzeSymbols({ component: componentName, inputOutput: InputOutput.Output }, symbolTableToSymbols(classSymbol.members));
        }
        const definition = {
            name: componentName,
            inputs: inputs,
    ...
    analyzeSymbols(context, symbols) {
        const properties = {};
    ...
    analyzeType(context, type, location, optional = false, docString = undefined) {
        ...
        if (type.isClassOrInterface()) {
            ...
            let name = type.getSymbol()?.escapedName;
            if (!name) {
            ...
            const originalTypeName = context.isPartial || context.isRequired ? type.getSymbol()?.escapedName : name;
        ...
        if (type.isUnion()) {
            ...
            const isStringLiteralUnion = unionType.types.every((t) => t.isStringLiteral() || (t.flags & ts.TypeFlags.Undefined) !== 0);
            const isNumberLiteralUnion = unionType.types.every((t) => t.isNumberLiteral() || (t.flags & ts.TypeFlags.Undefined) !== 0);
            if (isStringLiteralUnion || isNumberLiteralUnion) {
                // For `enum MyEnum { ... }` we have the name available directly on the type.
                let name = type.aliasSymbol?.escapedName;
                let typeRefNode;
    ...
    extractEnumMembers(context, type, location) {
        ...
        if (ts.isEnumDeclaration(declaration)) {
            ...
            for (const enumMember of declaration.members) {
                ...
                if (enumMember.initializer) {
                    if (ts.isStringLiteral(enumMember.initializer)) {
                        ...
                        throw new Error(`Unsupported initializer for enum member ${memberName} in ${this.formatErrorContext(context)}. Must be a string or numeric literal.`);
                    }
                }
                else {
                ...
                const doc = enumMember;
                if (doc?.jsDoc && doc.jsDoc.length > 0) {
                    memberDescription = doc.jsDoc.map((d) => d.comment).join("\n");
                }
            ...
            const docNode = declaration;
            let enumDescription;
            if (docNode?.jsDoc && docNode.jsDoc.length > 0) {
                enumDescription = docNode.jsDoc.map((doc) => doc.comment).join("\n");
            }
            ...
            if (initializer && ts.isObjectLiteralExpression(initializer) && type.isUnion()) {
                ...
                for (const property of initializer.properties) {
                    if (ts.isPropertyAssignment(property) && ts.isIdentifier(property.name)) {
                        ...
                        const propDocNode = property;
                        let enumDescription;
                        if (propDocNode?.jsDoc && propDocNode.jsDoc.length > 0) {
                            enumDescription = propDocNode.jsDoc.map((d) => d.comment).join("\n");
                        }
                ...
                if (variableStatement && ts.isVariableStatement(variableStatement)) {
                    const docNode = variableStatement;
                    if (docNode?.jsDoc && docNode.jsDoc.length > 0) {
                        description = docNode.jsDoc.map((doc) => doc.comment).join("\n");
                    }
                }
...
function getPartialInnerType(type) {
...
function getRequiredInnerType(type) {
